...
function factory_levels.update_machine_speed(machine, level, speed_multiplier)
...
function factory_levels.update_machine_module_slots(machine, level, levels_per_module_slot, base_module_slots,
													module_slot_bonus)
...
function factory_levels.update_machine_tint(machine, level)
	multiplier = level
		...
		for level = 0, machines.levels[tier], 1 do
			...
			if level > 0 then
				...
				machine.localised_name = { "entity-name.factory-levels",
					{ "entity-name." .. machines.base_machine_names[tier] }, level }
				machine.localised_description = { "entity-description." .. machines.base_machine_names[tier] }
			end

			factory_levels.update_machine_speed(machine, level,
				machines.speed_multipliers[tier])
			factory_levels.update_machine_energy_usage(machine, level, machines.base_consumption[tier],
				machines.consumption_multipliers[tier], machines.consumption_unit[tier])
			factory_levels.update_machine_pollution(machine, level, machines.base_pollution[tier],
				machines.pollution_multipliers[tier])
			factory_levels.update_machine_productivity(machine, level, machines.base_productivity[tier],
